...
**First goal:** get the order book *right* in hardware — no strategy, no order placement yet. The host feeds a recorded **Nasdaq ITCH** feed into the FPGA, the FPGA rebuilds the book in hardware, reports its state back, and the host checks it against its own software book.

## Architecture — Nexys4 DDR + Host (C++)
...
flowchart LR
    subgraph HOST["HOST CPU — Linux (C++)"]
        ...
        OB_REF["<b>OrderBook (reference)</b><br/>built from the same ITCH<br/>what the FPGA must match"]
    end
...
> Host sends feed over Ethernet → FPGA rebuilds book → reports back over UART → host compares.
> No strategy, no orders yet — first goal is just: get the OrderBook right in hardware.
